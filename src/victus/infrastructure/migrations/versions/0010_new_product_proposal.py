...
def upgrade() -> None:
    ...
    if op.get_bind().dialect.name == "postgresql":
        op.create_check_constraint("ck_product_proposal_kind", TABLE, "kind IN ('update','new')")
        op.create_foreign_key(
            "fk_product_proposal_consumable",
            TABLE,
            "consumable",
            ["consumable_id"],
            ["id"],
...
def downgrade() -> None:
    if op.get_bind().dialect.name == "postgresql":
        op.drop_constraint("fk_product_proposal_consumable", TABLE, type_="foreignkey")
        op.drop_constraint("ck_product_proposal_kind", TABLE, type_="check")
