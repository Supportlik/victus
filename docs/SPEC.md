...
| R83 | **A page is told when its data changed.** The client polled four counts a minute and the page in front of the reader refreshed never, so a draft written into the day that was open moved a badge while the day itself stayed stale. `GET /api/v1/events` is a server-sent stream for the authenticated tenant: `hello` on connect, `change` when the tenant's data moved, `heartbeat` against idle proxies, each message carrying `id:` = the cursor, which comes back as `Last-Event-ID` (or `?cursor=`) so a reconnect resumes instead of replaying. The change source is `audit_log`, whose `MAX(id)` per tenant is monotonic and cheap to ask for — an in-process event bus would miss the writes of the worker container and of MCP, which this sees. The **server** polls: one query per interval per connected client, each in its own unit of work, the loop ended by `request.is_disconnected()`. The four badge counts travel with every message and are computed in **one** application-layer place, so a count and the badge showing it cannot drift. **The audit `diff` never leaves the server:** a target is action, type and id, so a listener learns that the day changed, not what was eaten. `events.enabled` turns the stream off with a `503` rather than a connection that hangs. | `application/use_cases/events.py`, `api/routers/events.py`, `config/server.py::EventsConfig`, `infrastructure/db/repositories/ops.py::AuditRepo.max_id`/`since` | 3 |
...
| R36 | Audio is transcribed through a `TranscriptionPort` (first adapter: OpenAI `gpt-4o-transcribe`, configurable language and tenant vocabulary prompt). **Every** recording of a capture is transcribed, one transcript per audio part, each carrying the length the provider measured. | `application/ports/transcription.py`, `infrastructure/transcription/openai_transcribe.py`, `application/use_cases/captures.py::TranscribeCapture` | 3 |
...
| R81 | **Without `approve`, nothing is a fact.** An actor holding `write` may add to a day, but the item arrives as a draft; changing or removing an approved item, closing or reopening a day, setting `reliable`, writing the catalogue (product, version, portion) and deciding a proposal all require `approve`. A catalogue write attempted without it becomes a **proposal**: a food with no product yet is stored as a one-off consumable the day can log immediately, and approving the proposal *promotes* that consumable into the product, keeping every line item that already points at it. A portion travels the same road, one operation per entry — `add`, `update` or `delete` — so correcting or removing one costs no more scope than adding one, and the pending proposal carries what each entry would do to the catalogue and what would refuse it. A changed recipe is a `version` proposal: it carries `valid_from` beside the new values, and approving opens a version from that day (R70), so the days before it keep what they counted — which a correction to the current version would silently rewrite. An agent may withdraw its own draft (`agent:write`), never a fact. | `application/use_cases/_base.py::require_decision`, `day_logs.py` (`AddLineItem`, `UpdateLineItem`, `DeleteLineItem`, `UpdateDayFlags`, `CloseDay`, `ReopenDay`), `proposals.py` (`ProposeNewProduct`, `ProposeProductVersion`, `DecideProposal`, `create_or_propose_product`, `portion_plan`, `update_or_propose_portion`, `delete_or_propose_portion`, `version_or_propose_product_version`), `infrastructure/db/repositories/catalog.py::promote_ad_hoc`, migrations `0010_new_product_proposal` and `0012_product_version_proposal` | 3 |
| R82 | **A class is shown in the unit a person can act on.** Every threshold of a body scale carries its own value *and* the weight it corresponds to, plus the kilograms from here to there, so "obesity class II" also reads as "101.1–115.6 kg, 15.6 kg away". Both travel with the band rather than being paired up by position later. | `reports/results.py::ThresholdMark`, `reports/blocks/body.py::_bmi_marks`, `web/features/reports/report-blocks/report-block.ts` | 3 |
...
| R75 | **A portion is measured the way the product is, unless a density relates the two.** Nutrients are stated per 100 g or per 100 ml and are multiplied by the frozen base amount without the units being compared, so a portion in the other unit would count one for one — 400 ml of a syrup stated per 100 g as if it were 400 g. Such a portion is therefore refused unless the product carries a `density_g_per_ml`, and where it does, the density is applied: every resolved amount — a portion, a plain weight, a recipe ingredient — is converted into the product's own unit before it is frozen, so `base_unit` always equals the product's `reference_unit` and the arithmetic needs no unit check of its own. | `application/use_cases/products.py::_check_portion_unit`/`in_product_unit`, `application/use_cases/day_logs.py::resolve_base`, `domain/services/units.py::convert_base` | 1 |
...
| R78 | **The interface language is switchable at runtime.** `regional.language` (`en`, `de`, `es`, `fr`) changes the interface without a rebuild, kept separate from `regional.locale`, which only decides how numbers and dates are written, so German numbers with an English interface is a valid combination. The English string is its own key, so a missing translation falls back to correct English rather than showing a key. **The server sends no finished sentences either:** anything a person reads that the server produced — a day's findings, a report's notes, why a figure is missing — travels as a `Message` of key plus parameters (`{"key": "{n} days left", "params": {"n": 79}}`), and the interface writes it in its own language with its own number format. `scripts/check_translations.py` reads both halves, the `i18n.t()` calls in the templates and the `Message(...)` keys in the Python source, so a gap on either side is reported. | `domain/values.py::Message`, `reports/messages.py`, `web/core/i18n.service.ts::msg`, `web/core/i18n.{de,es,fr}.ts`, `scripts/check_translations.py` | 3 |
...
| `day_message` | The agent side of a day's thread (the day's verdict as one `summary`, questions, notes); captures are the user side |
