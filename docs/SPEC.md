...
| R78 | **The interface language is switchable at runtime.** `regional.language` (`en`, `de`) changes the interface without a rebuild, kept separate from `regional.locale`, which only decides how numbers and dates are written, so German numbers with an English interface is a valid combination. The English string is its own key, so a missing translation falls back to correct English rather than showing a key; `scripts/check_translations.py` reports the gaps so the fallback does not become permanent. | `web/core/i18n.service.ts`, `web/core/i18n.de.ts`, `scripts/check_translations.py` | 3 |
